...
控制台对每个 Session 保留一个尚未确认的提交，使用相同 request_key 和原始消息进行人工重发。网络错误、残缺 JSON 或 5xx 不代表服务端没有接收。切换会话不会丢弃该键；并发点击共享同一个 POST；收到有效 execution_id 后才完成确认，后续同样文本可作为新任务提交。这不是执行失败后的自动重试。

未确认状态只保存在当前页面内存。刷新页面、关闭页面或退出登录会清除它。此时应先查看该 Session 的任务历史确认是否已经执行，不能将新键重新发送当作跨页面的幂等恢复。存在未确认提交时，修改消息后发送会被阻止；重新选择会话可恢复待确认原文。

父 Agent 的 delegate 现在返回 output_ref、content_chars、最多512字符的前缀摘要以及截断标记。摘要标为 prefix_excerpt，不能当作完整结论；只有不超过2048字节的小 JSON 同时内联 result。完整输出及内容摘要标识保持不变。

确需更多细节时，父 Agent 可调用 read_child_result(execution_id, offset, limit)，每次最多2048个 Unicode 字符。只允许读取当前 Execution 的已完成直接子任务；不能拿任意对象引用读取其他任务。读取前检查租约与父子关系，存储读取后再次核验执行权限，再返回内容。child_result.summarized/read 只记录引用、位置、长度等元数据，不把大正文放进审计事件。

新增验证包括：原提交 HTTP 响应丢失后的跨会话重发、80,011字符子结果的摘要及尾部分页读取、未授权任务拒绝、旧执行代次和读取中权限失效、对象损坏检测。CI 在隔离检出中先用6e439fd原代码运行反例，再恢复新代码运行正向验收；反例预期失败不等于最终验收失败。
